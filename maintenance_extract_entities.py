...
class EntityExtractor:
    ...
    async def find_episodes_without_entities(
        ...
        records, _, _ = await self.driver.execute_query(query_with_entities, group_id=group_id)
        ...
        for record in records:
            ...
            if episode_data:
                ...
                episode = EpisodicNode(
                    ...
                    valid_at=episode_data.properties.get(
                        'valid_at', episode_data.properties.get('created_at')
                    ),
    ...
    async def process_episodes_batch(
        self, episodes: List[EpisodicNode], batch_size: int = 5
    ) -> Dict[str, any]:
        """Process a batch of episodes to extract entities"""
        results = {'processed': 0, 'entities_created': 0, 'failed': 0, 'entities_by_episode': {}}

        for i in range(0, len(episodes), batch_size):
            batch = episodes[i : i + batch_size]
            logger.info(
                f'Processing batch {i // batch_size + 1} of {(len(episodes) + batch_size - 1) // batch_size}'
            )
            ...
            for episode in batch:
                try:
                    ...
                    if entities:
                        ...
                        logger.info(
                            f'Created {len(entities)} entities for episode: {episode.content[:100]}...'
                        )
...
async def main():
    ...
    ollama_client = AsyncOpenAI(
        base_url=os.getenv('OLLAMA_BASE_URL', 'http://100.81.139.20:11434/v1'), api_key='ollama'
    )
    ...
    if total_orphans > 0:
        ...
        while True:
            ...
            print(f'\n{"=" * 60}')
            print(
                f'BATCH {batch_num} - Processing episodes {total_processed + 1} to {min(total_processed + 5, total_orphans)}'
            )
            print(f'{"=" * 60}')
        ...
        print(
            f'Average entities per episode: {total_entities_created / total_processed:.1f}'
