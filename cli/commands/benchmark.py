...
from graphiti_core.benchmarking import DryRunConfig, DryRunWorker, SafetyConfig, HyperparameterSpace
...
async def create_graphiti_instance(
    ...
    """Create a Graphiti instance with FalkorDB backend"""

    driver = FalkorDriver(host=falkor_host, port=falkor_port, database=falkor_database)
...
@click.option(
    '--enable-profiling/--disable-profiling', default=True, help='Enable performance profiling'
)
...
@click.option(
    ...
    type=int,
    default=30,
    help='Number of episodes per hyperparameter combination',
)
...
@click.option(
    '--entity-similarity', multiple=True, type=float, help='Entity similarity thresholds to test'
)
@click.option(
    '--output-dir', type=click.Path(), default='./hyperparameter_results', help='Output directory'
)
    ...
    async def _run_tuning():
        ...
        for i, hyperparams in enumerate(combinations):
            ...
            try:
                ...
                results.append(
                    {'combination': i + 1, 'hyperparameters': hyperparams, 'error': str(e)}
                )
            finally:
                await worker.close()
                await graphiti.close()
...
@benchmark.command()
@click.option('--episodes-limit', type=int, default=100, help='Number of episodes to benchmark')
@click.option(
    '--output-dir', type=click.Path(), default='./backend_comparison', help='Output directory'
)
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose logging')
def compare(episodes_limit: int, output_dir: str, verbose: bool):
    """Run performance benchmark on FalkorDB backend"""
    ...
    async def _run_comparison():
        click.echo(f'Running FalkorDB performance benchmark')
        click.echo(f'Episodes: {episodes_limit}')
        ...
        config = DryRunConfig(
            run_id=f'compare_falkor_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
            description='FalkorDB performance benchmark',
            episodes_limit=episodes_limit,
            batch_size=10,
            max_concurrent_episodes=3,
            output_dir=Path(output_dir) / 'falkor',
        ...
        try:
            result = await worker.run_benchmark()
            summary = result.get_summary_stats()
            results['FalkorDB'] = summary

            click.echo(f'  ✅ FalkorDB completed')
            click.echo(f'     Episodes/sec: {summary.get("episodes_per_second", 0):.2f}')
            click.echo(f'     Success rate: {summary.get("success_rate", 0) * 100:.1f}%')

        except Exception as e:
            click.echo(f'  ❌ FalkorDB failed: {e}')
            results['FalkorDB'] = {'error': str(e)}
        finally:
            await worker.close()
            await graphiti.close()
