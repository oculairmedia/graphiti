...
class EntityDeduplicator:
    ...
    async def find_duplicate_candidates(
        ...
        records, _, _ = await self.driver.execute_query(query, group_id=group_id)
        ...
        for i, entity1 in enumerate(entities):
            ...
            for j, entity2 in enumerate(entities[i + 1 :], i + 1):
                ...
                if entity1.name_embedding and entity2.name_embedding:
                    similarity = np.dot(
                        normalize_l2(entity1.name_embedding), normalize_l2(entity2.name_embedding)
                    )
    ...
    def _should_auto_merge(
        ...
        if len(duplicate_nodes) > 100:
            ...
            logger.info(
                f'Checking auto-merge for {len(duplicate_nodes)} nodes with {len(unique_normalized)} unique normalized names'
            )
        ...
        if len(duplicate_nodes) > 50:
            ...
            if match_ratio >= 0.95:
                logger.info(
                    f"Auto-merging {len(duplicate_nodes)} nodes: {match_ratio:.1%} exactly match '{base_name}'"
                )
                return True
            else:
                logger.info(
                    f"Not auto-merging {len(duplicate_nodes)} nodes: only {match_ratio:.1%} exactly match '{base_name}'"
                )
                return False
    ...
    async def merge_duplicate_group(
        ...
        logger.info(
            f'Processing {len(duplicate_nodes)} duplicate nodes: {[n.name for n in duplicate_nodes]}'
        )
    ...
    async def _update_edges_for_merged_nodes(self, primary_uuid: str, uuid_map: Dict[str, str]):
        ...
        await self.driver.execute_query(
            update_source_query, duplicate_uuids=duplicate_uuids, primary_uuid=primary_uuid
        )

        await self.driver.execute_query(
            update_target_query, duplicate_uuids=duplicate_uuids, primary_uuid=primary_uuid
        )
    ...
    async def _delete_duplicate_nodes(self, primary_uuid: str, uuid_map: Dict[str, str]):
        ...
        records, _, _ = await self.driver.execute_query(
            delete_query, duplicate_uuids=duplicate_uuids, primary_uuid=primary_uuid
        )
    ...
    async def run_deduplication(
        ...
        for i in range(0, len(duplicate_groups), batch_size):
            ...
            logger.info(
                f'Processing batch {i // batch_size + 1} of {(len(duplicate_groups) + batch_size - 1) // batch_size}'
            )
            ...
            for j, group in enumerate(batch):
                ...
                if dry_run:
                    ...
                    logger.info(
                        f'Processing group {group_index}/{total_groups} ({len(group)} entities to merge)'
                    )
                    primary_node, uuid_map = await self.merge_duplicate_group(group)
                    total_merged += len(uuid_map)
                    merge_results.append(
                        {'primary': primary_node.name, 'merged_count': len(uuid_map)}
                    )
                    group_end_time = time()
                    group_duration = group_end_time - group_start_time
                    logger.info(
                        f"Completed group {group_index}/{total_groups} in {group_duration:.2f}s - Merged {len(uuid_map)} into '{primary_node.name}'"
                    )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f'Centrality calculation request returned status {response.status_code}'
                    )
...
async def main():
    ...
    llm_client = OpenAIGenericClient(config=custom_llm_config, cache=False, client=ollama_client)
