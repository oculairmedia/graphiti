...
export function useGraphDataQuery() {
  ...
  // Debug logging only when data first loads (not on every count change)
  ...
  const transformedData = useMemo<TransformedData>(() => {
    ...
    console.log('[useGraphDataQuery] transformedData computing:', {
      isIncrementalUpdate,
      hasStableRef: !!stableTransformedDataRef.current,
      stableRefNodes: stableTransformedDataRef.current?.nodes?.length || 0,
    ...
    // BUT only if the stable ref actually has data
    if (isIncrementalUpdate && stableTransformedDataRef.current && stableTransformedDataRef.current.nodes.length > 0) {
