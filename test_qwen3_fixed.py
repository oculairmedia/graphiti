    ...
    try:
                    ...
                    if result:
                        if hasattr(result, 'nodes') and result.nodes:
                            ...
                            for node in result.nodes[:10]:  # Show up to 10
                                print(f"      • {node.name} ({', '.join(node.labels) if hasattr(node, 'labels') else 'Entity'})")
        ...
        if successful_episodes:
            ...
            if node_result and node_result[0]:
                count = node_result[0][0]['count'] if isinstance(node_result[0][0], dict) else node_result[0][0][0]
            ...
            if entity_result and entity_result[0]:
                count = entity_result[0][0]['count'] if isinstance(entity_result[0][0], dict) else entity_result[0][0][0]
            ...
            if rel_result and rel_result[0]:
                count = rel_result[0][0]['count'] if isinstance(rel_result[0][0], dict) else rel_result[0][0][0]
            ...
            if rels and rels[0] and len(rels[0]) > 0:
                for row in rels[0][:10]:
                    if isinstance(row, dict):
                        print(f"   • {row.get('source', '?')} --[{row.get('rel_type', '?')}]--> {row.get('target', '?')}")
                        if row.get('fact'):
