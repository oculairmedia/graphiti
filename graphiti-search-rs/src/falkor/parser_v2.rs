...
/// Parse edges with properties from FalkorDB LazyResultSet (for similarity search workaround)
pub fn parse_edges_with_properties_from_falkor_v2(result: LazyResultSet<'_>) -> Result<Vec<Edge>> {
    let mut edges = Vec::new();

    // Iterate over the lazy result set
    for row in result {
        // We expect: [source_node, edge_uuid, edge_fact, edge_created_at, edge_group_id, edge_weight, target_node, score]
        if row.len() >= 7 {
            if let (
