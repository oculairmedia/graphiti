...
async fn test_falkordb_sdk_similarity_search() -> Result<()> {
    ...
    for (i, row) in data.iter().enumerate().take(3) {
        if row.len() >= 3 {
            if let (Some(FalkorValue::String(name)), Some(FalkorValue::String(_uuid)), Some(score_val)) = 
    ...
    for (i, row) in data.iter().enumerate().take(3) {
        if row.len() >= 3 {
            if let (Some(FalkorValue::String(fact)), Some(FalkorValue::String(_uuid)), Some(score_val)) = 
                (row.get(0), row.get(1), row.get(2)) {
                let score = match score_val {
                    FalkorValue::F64(f) => *f,
                    FalkorValue::I64(i) => *i as f64,
...
async fn test_falkordb_client_v2() -> Result<()> {
    ...
    let results = client.similarity_search_nodes(&test_embedding, 5, 0.0).await?;
    println!("✓ Node similarity search returned {} results", results.len());
    
    let edge_results = client.similarity_search_edges(&test_embedding, 5, 0.0).await?;
    println!("✓ Edge similarity search returned {} results", edge_results.len());
