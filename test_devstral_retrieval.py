...
async def test_retrieval():
    ...
    driver = FalkorDriver(host='localhost', port=6389, database='devstral_test')
    ...
    try:
        ...
        result = await driver.execute_query('MATCH (n:Entity) RETURN count(n) as count')
        if result:
            count = result[0]['count'] if isinstance(result[0], dict) else result[0][0]
        ...
        result = await driver.execute_query('MATCH ()-[r]->() RETURN count(r) as count')
        if result:
            count = result[0]['count'] if isinstance(result[0], dict) else result[0][0]
