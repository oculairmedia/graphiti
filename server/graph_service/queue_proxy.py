...
class QueueProxy:
    ...
    async def send_to_queue(self, task: Dict[str, Any], visibility_timeout: int = 0) -> bool:
    ...
    async def send_message_to_queue(self, message: Message, group_id: str) -> bool:
        ...
        # don't create a brand-new episode.
        episode_uuid = message.uuid or str(uuid.uuid4())

        # Generate a sensible name if not provided
        episode_name = (
            message.name
            if message.name
        ...
        task = {
            ...
            'metadata': {'source': 'api', 'created_at': datetime.utcnow().isoformat()},
        }
    ...
    async def send_relationship_to_queue(
        self,
        ...
        task = {
            ...
            'payload': {'source_node': source_node, 'edge': edge, 'target_node': target_node},
            'group_id': group_id,
            'priority': 1,  # TaskPriority.NORMAL (numeric value)
            'retry_count': 0,
            'max_retries': 3,
    ...
    async def send_deduplication_to_queue(
        self, dedup_type: str, group_ids: list[str], similarity_threshold: float = 0.8
    ) -> bool:
