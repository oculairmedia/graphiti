"""
DEPRECATED: Queue-based ingestion router for Graphiti.
Use /api/temporal/messages endpoint instead for Temporal-native ingestion.

Legacy: Uses Wilson Lin's queued service for high-performance, reliable ingestion.
"""

import warnings

warnings.warn(
    'ingest_queue router is deprecated. Use /api/temporal/messages endpoint instead.',
    DeprecationWarning,
    stacklevel=2,
)
...
def get_queue_client() -> QueuedClient:
    ...
    if _queue_client is None:
        _queue_client = QueuedClient(
            base_url='http://graphiti-queued:8080'
...
async def get_task_status(task_id: str):
    ...
    return QueueStatus(task_id=task_id, status='processing', message='Task is being processed')
...
async def get_queue_stats():
    ...
    try:
        ...
        return {'queues': queues, 'metrics': stats, 'status': 'healthy'}

    except Exception as e:
        logger.error(f'Failed to get queue stats: {e}')
        return {'queues': [], 'metrics': {}, 'status': 'error', 'error': str(e)}
    ...
    try:
        ...
        while True:
            ...
            messages = await client.poll(
                queue_name=queue_name, count=batch_size, visibility_timeout=1
            )
