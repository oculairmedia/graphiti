...
class SearchWithRelevanceRequest(BaseModel):
    query: str = Field(description='Search query')
    group_ids: Optional[list[str]] = Field(default=None)
    include_relevance_scores: bool = Field(default=True)
    apply_relevance_filter: bool = Field(default=True)
    auto_update_scores: bool = Field(default=False)


class SearchWithRelevanceResponse(BaseModel):
...
async def get_scorer(
    graphiti: ZepGraphitiDep, llm_client: Optional[LLMClient] = Depends(create_llm_client)
) -> RelevanceScorer:
...
async def submit_relevance_feedback(
    ...
    try:
        ...
        return RelevanceFeedbackResponse(
            status='success', processed_count=len(feedbacks), feedbacks=feedbacks
        )
...
async def get_memories_with_scores(
    ...
    try:
        ...
        records, _, _ = await driver.execute_query(
            query, group_id=group_id, min_relevance=min_relevance, limit=limit, routing_='r'
        )
        ...
        for record in records:
            memories.append(
                MemoryWithScoreResponse(
                    ...
                    last_accessed=str(record.get('last_accessed'))
...
async def bulk_recalculate_scores(
    ...
    try:
        ...
        if not memory_ids and request.group_id:
            ...
            """
            records, _, _ = await driver.execute_query(
                query, group_id=request.group_id, routing_='r'
            )
            memory_ids = [r['memory_id'] for r in records]

        if not memory_ids:
            return BulkRecalculateResponse(
                status='no_memories', processed_count=0, failed_count=0, results=[]
            )
        ...
        for memory_id in memory_ids:
            try:
                ...
                failed_count += 1
                results.append({'memory_id': memory_id, 'status': 'failed', 'error': str(e)})
...
async def apply_rrf_fusion(
    ...
    try:
        ...
        # Format response
        response = [
            {'memory_id': memory_id, 'rrf_score': score, 'rank': rank}
...
@router.post('/search/with-relevance', response_model=SearchWithRelevanceResponse)
async def search_with_relevance(
    request: SearchWithRelevanceRequest,
    graphiti: ZepGraphitiDep,
    llm_client: Optional[LLMClient] = Depends(create_llm_client),
) -> SearchWithRelevanceResponse:
    """Search graph memory with relevance-enhanced ranking and filtering."""
    try:
        effective_llm_client = llm_client or graphiti.llm_client
        clients = GraphitiClients(
            driver=cast(Any, graphiti.driver),
            llm_client=cast(Any, effective_llm_client),
            embedder=graphiti.embedder,
            cross_encoder=getattr(graphiti, 'cross_encoder', None),
        )
        relevance_search = RelevanceEnhancedSearch(cast(Any, clients))
        results = await relevance_search.search_with_relevance(
            query=request.query,
            group_ids=request.group_ids,
            include_relevance_scores=request.include_relevance_scores,
            apply_relevance_filter=request.apply_relevance_filter,
            auto_update_scores=request.auto_update_scores,
        )
        return SearchWithRelevanceResponse(
        ...
        logger.error(f'Error in relevance-enhanced search: {e}')
        raise HTTPException(status_code=500, detail=str(e))
