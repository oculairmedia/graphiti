...
class WebhookService:
    ...
    async def add_internal_handler(
        self, handler: Callable[[NodeAccessEvent], Union[None, Awaitable[None]]]
    ) -> None:
    ...
    async def emit_node_access(
        ...
        logger.info(
            f'emit_node_access called: enabled={self._enabled}, node_ids={len(node_ids) if node_ids else 0}, handlers={len(self.internal_handlers)}'
        )
    ...
    async def emit_data_ingestion(
        ...
        if tasks:
            logger.debug('Sending data ingestion webhooks to %d endpoints', len(tasks))
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error('Data webhook to %s failed: %s', webhook_urls[i], result)
    ...
    async def _send_data_webhook(self, url: str, event: DataIngestionEvent) -> None:
        ...
        try:
            ...
            if response.status_code >= 400:
                logger.error(
                    f'Data webhook to {url} failed with status {response.status_code}: {response.text}'
                )
    ...
    async def _call_handler(
        self,
        handler: Callable[[NodeAccessEvent], Union[None, Awaitable[None]]],
        event: NodeAccessEvent,
    ) -> None:
