...
async def check_extraction_results(episode_content: str):
    ...
    records, _, _ = await driver.execute_query(
        query, group_id=TEST_GROUP_ID, content_snippet=episode_content[:50]
    )
    ...
    entity_records, _, _ = await driver.execute_query(entity_query, uuid=episode['uuid'])
...
async def test_with_server_logs():
    ...
    for i, message in enumerate(test_messages):
        ...
        if response:
            ...
            results.append(
                {'message': message[:100], 'entities_found': entity_count, 'response': response}
            )
        else:
            results.append(
                {
    ...
    for i, result in enumerate(results):
        ...
        logger.info(
            f'{status} Test {i + 1}: {result["entities_found"]} entities - {result["message"]}...'
        )
