                ...
                for j, node in enumerate(nodes_to_embed):
                    ...
                    try:
                        _, _, _ = await driver.execute_query(
                            update_query, uuid=node['uuid'], embedding=embeddings[j]
                        )
                        updated += 1
        ...
        if edges_to_embed:
            ...
            try:
                ...
                for j, edge in enumerate(edges_to_embed):
                    ...
                    try:
                        _, _, _ = await driver.execute_query(
                            update_query, uuid=edge['uuid'], embedding=embeddings[j]
                        )
                        updated += 1
...
async def main():
    ...
    try:
        ...
        results.append({'type': 'Nodes', 'updated': nodes_updated, 'failed': nodes_failed})
    except Exception as e:
        print_error(f'Failed to fix node embeddings: {e}')
        results.append({'type': 'Nodes', 'updated': 0, 'failed': -1})

    # Fix missing edge embeddings
    try:
        edges_updated, edges_failed = await fix_missing_edge_embeddings(driver, embedder)
        results.append({'type': 'Edges', 'updated': edges_updated, 'failed': edges_failed})
    except Exception as e:
        print_error(f'Failed to fix edge embeddings: {e}')
        results.append({'type': 'Edges', 'updated': 0, 'failed': -1})
