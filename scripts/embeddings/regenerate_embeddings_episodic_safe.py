#!/usr/bin/env python3
"""
SAFE Episodic node embedding regeneration - Conservative approach.

Same safety features as Entity script but for Episodic nodes.
"""
...
async def safe_batch_update(
    ...
    """Update Episodic nodes with embeddings using UNWIND."""
    batch_data = [
    ...
    MATCH (n:Episodic {uuid: item.uuid})
...
async def main():
    ...
    BATCH_SIZE = 25
    BATCH_DELAY = 0.2
    CHECKPOINT_FILE = "/tmp/embedding_progress_episodic.json"

    logger.info("=" * 80)
    logger.info("SAFE EPISODIC EMBEDDING REGENERATION - Conservative Mode")
    logger.info("=" * 80)
    ...
    # Count total nodes - Episodic nodes use 'content' field for embedding
    count_query = "MATCH (n:Episodic) WHERE n.content IS NOT NULL RETURN count(n) as total"
    result, _, _ = await driver.execute_query(count_query)
    total = result[0]['total'] if result else 0

    logger.info(f"📊 Found {total} Episodic nodes to process\n")
    ...
    for offset in range(0, total, BATCH_SIZE):
        ...
        # Fetch batch - Episodic nodes have 'content' field
    ...
    logger.info("=" * 80)
    logger.info("✅ EPISODIC PROCESSING COMPLETE!")
    logger.info("=" * 80)
