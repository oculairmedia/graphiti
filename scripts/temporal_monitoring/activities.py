...
class GraphHealthCheckOutput:
    ...
    episodes_per_minute_1h: float  # Rate over last hour
    episodes_per_minute_5m: float  # Rate over last 5 minutes
    status: str
    alerts: list[str]
...
class MonitoringActivities:
    ...
    async def check_graph_health(self, input: MonitoringInput) -> GraphHealthCheckOutput:
        ...
        recent_5m_result = self._query(
            f"MATCH (e:Episodic) WHERE e.created_at > '{five_min_ago}' RETURN count(e) as c"
        )
        recent_5m_episodes = recent_5m_result[0]['c'] if recent_5m_result else 0

        episodes_per_minute_1h = recent_episodes / 60.0 if recent_episodes > 0 else 0.0
        episodes_per_minute_5m = recent_5m_episodes / 5.0 if recent_5m_episodes > 0 else 0.0
        ...
        logger.info(
            'Graph health check: status=%s isolated=%d ratio=%.2f eps/min(1h)=%.2f eps/min(5m)=%.2f alerts=%s',
            status,
            isolated_count,
            edge_ratio,
            episodes_per_minute_1h,
            episodes_per_minute_5m,
            alerts,
        )
        ...
        return GraphHealthCheckOutput(
            ...
            episodes_per_minute_1h=episodes_per_minute_1h,
            episodes_per_minute_5m=episodes_per_minute_5m,
            status=status,
            alerts=alerts,
        )
