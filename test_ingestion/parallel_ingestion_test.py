"""
Parallel Ingestion Pipeline Test Script
...
Usage:
    python test_ingestion/parallel_ingestion_test.py
...
from graphiti_core.prompts.extract_nodes import ExtractedEntities, ExtractedEntity, MissedEntities
...
class OllamaLLMClient:
    ...
    async def generate_response(self, prompt: str, response_model=None) -> Dict[str, Any]:
            ...
            async with self.session.post(f"{self.base_url}/api/generate", json=payload) as response:
                ...
                try:
                    ...
                    if response_model == ExtractedEntities:
                        ...
                        return {
                            "extracted_entities": [
                                {"name": "Test Entity 1", "entity_type_id": 0, "summary": "Test entity"},
                                {"name": "Test Entity 2", "entity_type_id": 0, "summary": "Another test entity"}
                            ]
                        }
                    elif response_model == MissedEntities:
                    ...
                    return {"extracted_entities": []} if response_model == ExtractedEntities else {}
    ...
    async def extract_entities_sequential(self, episode_content: str, max_iterations: int = 3) -> PerformanceMetrics:
        ...
        async with OllamaLLMClient(self.ollama_url, self.model) as client:
            context = {
                ...
                'episode_timestamp': utc_now().isoformat(),
                'previous_episodes': [],
                'custom_prompt': '',
            ...
            entities = [ExtractedEntity(**entity_data) for entity_data in initial_response.get('extracted_entities', [])]
            ...
            for iteration in range(max_iterations):
                ...
                if iteration < max_iterations - 1:
                    additional_entities = [
                        ExtractedEntity(name=name, entity_type_id=0, summary=f"Additional entity {iteration}")
                        for name in missed_entities[:2]  # Limit for testing
                    ]
    ...
    async def extract_entities_parallel(self, episode_content: str, max_concurrent: int = 3) -> PerformanceMetrics:
        ...
        async with OllamaLLMClient(self.ollama_url, self.model) as client:
            ...
            for i, result in enumerate(initial_results):
                ...
                entities = [ExtractedEntity(**entity_data) for entity_data in result.get('extracted_entities', [])]
                all_entities.extend(entities)
            ...
            if reflexion_tasks:
                ...
                for i, result in enumerate(reflexion_results):
                    ...
                    if missed_entities:
                        ...
                        additional_entities = [
                            ExtractedEntity(name=name, entity_type_id=0, summary=f"Parallel entity {i}")
                            for name in missed_entities[:2]
                        ]
                        all_entities.extend(additional_entities)
