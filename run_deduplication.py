...
async def find_potential_duplicates(graphiti: Graphiti, group_id: str = None):
    ...
    for i, record in enumerate(records):
        ...
        for similar_node in search_results.nodes:
            ...
            if (similar_node.name.lower() == node_name.lower() or
                similar_node.name.lower() in node_name.lower() or
                node_name.lower() in similar_node.name.lower()):
                ...
                duplicates_found.append({
                    ...
                    'score': search_results.metadata.get('scores', {}).get(similar_node.uuid, 0)
                })
            ...
            if primary_node and dup_node:
                ...
                if not existing_check[0]:  # No existing edge
                    ...
                    duplicate_edges = build_duplicate_of_edges(
                        EpisodicNode(name="Deduplication Maintenance", uuid=str(uuid4()), group_id=primary_node.group_id),
                        now,
...
async def main():
    ...
    if duplicates:
        ...
        for i, dup in enumerate(duplicates[:20]):  # Show first 20
            print(f"{i+1}. '{dup['node1_name']}' <-> '{dup['node2_name']}' (score: {dup['score']:.3f})")
