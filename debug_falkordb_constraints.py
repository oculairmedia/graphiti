...
async def debug_falkordb_constraints():
    ...
    try:
        ...
        try:
            ...
            try:
                falkor_graph.query("""
                    MATCH (s {uuid: 'test-source-uuid'}), (t {uuid: 'test-target-uuid'})
            ...
            # Try creating relationship WITH uuid but no group_id
            try:
                falkor_graph.query("""
                    MATCH (s {uuid: 'test-source-uuid'}), (t {uuid: 'test-target-uuid'})
            ...
            # Try creating relationship WITH both uuid and group_id
            try:
                falkor_graph.query("""
                    MATCH (s {uuid: 'test-source-uuid'}), (t {uuid: 'test-target-uuid'})
