...
async def test_simple_ingestion():
    ...
    falkor_driver = FalkorDriver(host='localhost', port=6389, database='simple_ingestion_test')

    # Create Graphiti with dummy components
    graphiti = Graphiti(
        graph_driver=falkor_driver, embedder=DummyEmbedder(), llm_client=DummyLLMClient()
    )
