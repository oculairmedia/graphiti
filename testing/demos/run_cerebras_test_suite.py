#!/usr/bin/env python3
"""
Comprehensive Cerebras/Qwen Test Suite Runner
Executes all Cerebras tests in sequence and generates a comprehensive report.
"""
...
class CerebrasTestSuiteRunner:
    """Orchestrates the complete Cerebras/Qwen test suite."""
        ...
        self.test_files = [
            {
                ...
                'file': 'test_minimal_cerebras.py',
                'description': 'Basic connectivity and client initialization',
                'critical': True,
                'timeout': 120
            },
            {
                'name': 'Structured Output Test', 
                'file': 'test_cerebras_structured.py',
                'description': 'JSON structure and Qwen-specific capabilities',
                'critical': True,
                'timeout': 180
            },
            {
                'name': 'Debug and Diagnostics',
                'file': 'test_cerebras_debug.py', 
                'description': 'Component-by-component debugging',
                'critical': False,
                'timeout': 300
            },
            {
                'name': 'Full Integration Test',
                'file': 'test_full_cerebras.py',
                'description': 'Complete Graphiti integration with Qwen',
                'critical': True,
                'timeout': 600
            },
            {
                'name': 'Ingestion Pipeline Test',
                'file': 'test_cerebras_ingestion.py',
                'description': 'AI research data ingestion and extraction',
                'critical': True,
                'timeout': 900
            },
            {
                'name': 'Retrieval and Search Test',
                'file': 'test_cerebras_retrieval.py',
                'description': 'Data retrieval and search functionality',
                'critical': True,
                'timeout': 600
            },
            {
                'name': 'Parameter Optimization',
                'file': 'test_cerebras_optimization.py',
                'description': 'Temperature and token optimization testing',
                'critical': False,
                'timeout': 900
            },
            {
                'name': 'Cerebras vs Ollama Comparison',
                'file': 'test_cerebras_vs_ollama_comparison.py',
                'description': 'Head-to-head performance comparison',
                'critical': False,
                'timeout': 1200
            },
            {
                'name': 'End-to-End Pipeline Integration',
                'file': 'test_cerebras_pipeline_integration.py',
                'description': 'Complete pipeline integrity testing',
                'critical': True,
                'timeout': 1800
            }
        ]
    ...
    def check_prerequisites(self) -> Dict[str, bool]:
        ...
        if checks['cerebras_api_key']:
            print('   ✅ CEREBRAS_API_KEY is set')
        else:
            print('   ❌ CEREBRAS_API_KEY not found - LLM tests will fail')
        
        # Check FalkorDB connection (simple ping)
        try:
        ...
        # Check Ollama connection (for hybrid embeddings)
        try:
            import aiohttp
    ...
    async def run_test_suite(self, skip_non_critical: bool = False) -> Dict[str, Any]:
        """Run the complete test suite."""
        
        print('🧠 Cerebras/Qwen Test Suite Runner')
        print('=' * 80)
        print(f'Start Time: {datetime.now().isoformat()}')
        print(f'Model: {os.getenv("CEREBRAS_MODEL", "qwen-3-coder-480b")}')
        ...
        if not prereqs['cerebras_api_key']:
            print('⚠️ WARNING: No CEREBRAS_API_KEY - many tests will fail')
    ...
    def print_comprehensive_report(self):
        ...
        print('\n📋 CEREBRAS/QWEN TEST SUITE COMPREHENSIVE REPORT')
        print('=' * 90)
        ...
        if non_critical_results:
            non_critical_passed = len([r for r in non_critical_results if r.success])
            non_critical_rate = (non_critical_passed / len(non_critical_results) * 100)
            print(f'   Optional Tests: {non_critical_passed}/{len(non_critical_results)} passed ({non_critical_rate:.1f}%)')
        
        # Detailed results
        print(f'\n📋 Detailed Results:')
        print(f'{"Test Name":<35} {"Status":<10} {"Duration":<10} {"Notes":<20}')
        print('-' * 90)
        ...
        for result in self.results:
            ...
            print(f'{result.test_name:<35} {status:<10} {result.duration:>8.1f}s {notes:<20}')
        ...
        if success_rate >= 90:
            print('   ✅ Excellent! Cerebras/Qwen integration is production-ready')
        elif success_rate >= 75:
            print('   ⚠️ Good progress, but address remaining failures before production')
        elif success_rate >= 50:
            print('   ⚠️ Significant issues detected - thorough debugging needed')
        else:
            print('   ❌ Major problems - review setup and configuration')
        
        # Specific recommendations based on failures
        if any('timeout' in (r.error or '').lower() for r in failed_results):
            print('   🔧 Consider increasing timeout values for slow operations')
            
        if any('api' in (r.error or '').lower() for r in failed_results):
            print('   🔧 Verify CEREBRAS_API_KEY and account limits')
            
        if any('connection' in (r.error or '').lower() for r in failed_results):
            print('   🔧 Check FalkorDB and Ollama service availability')
        
        # Next steps
        print(f'\n🎯 Next Steps:')
        if failed_results:
            print('   1. Review failed test logs for specific issues')
            print('   2. Fix configuration and connectivity problems')
            print('   3. Re-run failed tests individually for debugging')
        else:
            print('   1. Run performance benchmarks')
            print('   2. Test with production data volume')
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Run Cerebras/Qwen test suite')
    ...
    runner = CerebrasTestSuiteRunner()
    
    # Adjust timeouts if requested
    ...
    # Run the test suite
    summary = await runner.run_test_suite(skip_non_critical=args.critical_only)
