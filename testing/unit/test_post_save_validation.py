...
def create_mock_driver():
    ...
    async def mock_run(query, **params):
        ...
        if 'source_count' in query or 'target_count' in query:
            record = {'source_count': 1, 'target_count': 1}
            result.single.return_value = record
            result.__aiter__ = MagicMock(return_value=iter([record]))
        elif 'UNION' in query:
            records = [{'count': 1}, {'count': 0}]
            result.single.return_value = records[0]
            result.__aiter__ = MagicMock(return_value=iter(records))
        else:
            record = {'count': 1}
            result.single.return_value = record
            result.__aiter__ = MagicMock(return_value=iter([record]))
...
UNLABELED_MATCH_RE = re.compile(r'MATCH\s+\(\w+\s*\{uuid:', re.IGNORECASE)


async def test_queries_use_labeled_match():
    """Regression: all uuid MATCH queries must specify a node label for index usage."""
    driver = create_mock_driver()
    validator = PostSaveValidator(driver)

    entity = create_test_entity('Label Test')
    await validator.validate_entity_post_save(entity)

    edge = create_test_edge(str(uuid4()), str(uuid4()))
    await validator.validate_edge_post_save(edge)

    session = driver.session.return_value
    for call in session.run.call_args_list:
        query = call[0][0] if call[0] else call[1].get('query', '')
        assert not UNLABELED_MATCH_RE.search(query), f'Unlabeled MATCH found (full scan): {query}'
        if 'uuid' in query.lower():
            assert ':Entity' in query or ':Episodic' in query, f'MATCH missing label: {query}'


async def test_queries_use_parameterized_uuids():
    """Regression: uuid values must use $param not f-string interpolation."""
    driver = create_mock_driver()
    validator = PostSaveValidator(driver)

    entity = create_test_entity('Param Test', uuid='injected-uuid-value')
    await validator.validate_entity_post_save(entity)

    session = driver.session.return_value
    for call in session.run.call_args_list:
        query = call[0][0] if call[0] else call[1].get('query', '')
        assert 'injected-uuid-value' not in query, (
            f'UUID value interpolated into query (injection risk): {query}'
        )
...
async def test_entity_existence_check():
    ...
    assert existence_results[0].passed, (
        f'Entity existence check should pass: {existence_results[0].message}'
    )
...
async def test_edge_reference_check():
    ...
    assert reference_results[0].passed, (
        f'Edge reference check should pass: {reference_results[0].message}'
    )
...
async def test_uuid_uniqueness_check():
    ...
    assert uniqueness_results[0].passed, (
        f'UUID uniqueness check should pass: {uniqueness_results[0].message}'
    )
...
async def test_centrality_bounds_check():
    ...
    assert centrality_results[0].passed, (
        f'Valid centrality values should pass: {centrality_results[0].message}'
    )
    print('✅ Valid centrality bounds check passed')

    # Test invalid centrality values
    entity_invalid = create_test_entity(
        'Invalid Centrality Entity',
...
async def test_required_fields_check():
    ...
    assert required_results[0].passed, (
        f'Complete entity should pass required fields check: {required_results[0].message}'
    )
...
async def test_embedding_consistency_check():
    ...
    entity = create_test_entity('Test Entity', name_embedding=[0.1, 0.2, 0.3, 0.4])
    ...
    assert embedding_results[0].passed, (
        f'Consistent embeddings should pass: {embedding_results[0].message}'
    )
...
async def test_temporal_consistency_check():
    ...
    entity = create_test_entity(
        'Test Entity', created_at=base_time, updated_at=base_time + timedelta(minutes=30)
    )
    ...
    assert temporal_results[0].passed, (
        f'Consistent timestamps should pass: {temporal_results[0].message}'
    )
    print('✅ Consistent temporal check passed')

    # Test entity with inconsistent timestamps
    entity_inconsistent = create_test_entity(
        'Inconsistent Entity',
        created_at=datetime.now(),
...
async def run_all_tests():
    ...
    try:
        await test_queries_use_labeled_match()
        await test_queries_use_parameterized_uuids()
