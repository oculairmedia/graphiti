...
async def test_validation_service_initialization():
    ...
    config = ValidationConfig(
        enable_pre_save_validation=False, batch_size=50, fail_on_warnings=True
    )
...
async def test_entity_comprehensive_validation():
    ...
    entities = [
        ...
        create_test_entity(
            'Invalid Centrality Entity', degree_centrality=1.5
        ),  # Invalid centrality
...
async def test_validation_report_functionality():
    ...
    report = ValidationReport(
        operation_id='test_op', timestamp=datetime.now(), total_entities=10, total_edges=5
    )
    ...
    assert report.error_count == 2, (
        f'Should count 2 errors (ERROR + CRITICAL), got {report.error_count}'
    )
...
async def test_performance_metrics():
    ...
    assert report.performance_metrics['entities_per_second'] > 0, (
...
async def test_validation_summary():
    ...
    print(
        f'Summary: {summary["total_entities"]} entities, {summary["total_errors"]} errors, {summary["total_warnings"]} warnings'
    )
