    ...
    async def mock_run(query, **params):
        ...
        # Duplicate entities query
            ...
            mock_record = MockRecord(
                {'duplicate_groups': 3, 'duplicate_entities': 8, 'sample_groups': sample_groups}
            )
...
async def test_entity_count_monitoring():
    ...
    assert alerts[0].severity in [MonitoringSeverity.WARNING, MonitoringSeverity.ERROR], (
...
async def test_duplicate_entity_detection():
    ...
    assert alerts[0].metric == MonitoringMetric.DUPLICATE_ENTITIES, (
        'Should be duplicate entities metric'
    )
...
async def test_missing_embeddings_detection():
    ...
    assert alerts[0].metric == MonitoringMetric.MISSING_EMBEDDINGS, (
        'Should be missing embeddings metric'
    )
...
async def test_full_monitoring_cycle():
    ...
    assert health_report.overall_health in ['healthy', 'warning', 'error', 'critical'], (
        'Should have valid health status'
    )
    assert health_report.total_alerts >= 0, 'Should have non-negative alert count'
    print(
        f'✅ Full monitoring cycle works - Health: {health_report.overall_health}, Alerts: {health_report.total_alerts}'
    )
...
async def test_health_summary():
    ...
    alerts = [
        ...
        MonitoringAlert(
            ...
            MonitoringMetric.ORPHANED_EDGES,
            'Error alert',
        ),
        MonitoringAlert(
            'warn1',
            ...
            MonitoringMetric.DUPLICATE_ENTITIES,
            'Warning alert',
        ),
        MonitoringAlert(
            'warn2',
            datetime.now(),
            MonitoringSeverity.WARNING,
            MonitoringMetric.MISSING_EMBEDDINGS,
            'Another warning',
        ),
    ]
    ...
    assert summary['overall_status'] in ['critical', 'error', 'warning'], (
        'Should reflect alert severity'
    )

    print(
        f'✅ Health summary works - Score: {summary["health_score"]}, Status: {summary["overall_status"]}'
    )
...
async def test_configuration():
    ...
    try:
        ...
        assert thresholds.max_entity_count_change_percent == 15.0, (
            'Should load entity count threshold from env'
        )
        assert thresholds.max_orphaned_edges_percent == 2.5, (
            'Should load orphaned edges threshold from env'
        )
