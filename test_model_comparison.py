...
async def extract_with_model(episode: EpisodicNode, model_name: str) -> Dict[str, any]:
    ...
    try:
        ...
        llm_response = await llm_client.generate_response(
            prompt_library.extract_nodes.extract_message(context), response_model=ExtractedEntities
        )
        ...
        for entity_data in extracted_entities:
            entities.append(
                {'name': entity_data.get('name', ''), 'labels': ['Entity'], 'uuid': str(uuid4())}
            )
        ...
        logger.info(
            f'Model {model_name}: Extracted {len(entities)} entities in {extraction_time:.2f} seconds'
        )
...
Claude has been helping debug issues with zoom functionality not working after page refresh, implementing proper canvas readiness detection, and ensuring the visualization works correctly with the graph data."""
    ...
    for result in results:
        if result['success']:
            print(
                f'{result["model"]:<15} {result["extraction_time_seconds"]:<15.2f} {result["entities_count"]:<20} Success'
            )
        else:
            print(
                f'{result["model"]:<15} {result["extraction_time_seconds"]:<15.2f} {"N/A":<20} Failed: {result["error"][:30]}...'
            )
