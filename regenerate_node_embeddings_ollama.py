#!/usr/bin/env python3
"""
Regenerate embeddings for all nodes using Ollama.
This script will regenerate embeddings for ALL nodes, not just missing ones.
...
async def regenerate_node_embeddings(
    dry_run: bool = False, 
    batch_size: int = 50, 
    ...
    print("\n" + "="*80)
    print("NODE EMBEDDING REGENERATION WITH OLLAMA")
    print("="*80 + "\n")
    ...
    print("\nStep 1: Counting nodes to process...")
    print("-" * 50)
    ...
    if force_regenerate:
        ...
        """
        description = "nodes with names (will regenerate ALL)"
    else:
        count_query = """
        MATCH (n:Entity)
        ...
        RETURN count(n) as total
        """
        description = "nodes with names but no embeddings"
    ...
    if total_nodes == 0:
        print("No nodes need processing!")
        await driver.close()
        return
    ...
    print("\nStep 2: Processing nodes in batches...")
    print("-" * 50)
    ...
    while processed < nodes_to_process:
        ...
        if not dry_run:
            ...
            for result in results:
                print(f"  Would generate embedding for node {result['uuid'][:8]}... ({result['name']})")
    ...
    # Final report
    print("\n" + "="*80)
    print("NODE EMBEDDING REGENERATION COMPLETE")
    print("="*80 + "\n")
    ...
    if not dry_run:
        ...
        print(f"  Total nodes with embeddings: {total_with_embeddings}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Regenerate node embeddings using Ollama')
    ...
    try:
        updated, failed, remaining = await regenerate_node_embeddings(
