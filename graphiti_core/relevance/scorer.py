...
class ScoringContext(BaseModel):
    ...
    decomposed_query: Optional[str] = Field(
        default=None, description='Decomposed query sent to Graphiti'
    )
...
class RelevanceScorer:
    ...
    async def score_memory_llm(self, context: ScoringContext) -> float:
        ...
        try:
            ...
            logger.debug(
                f'LLM scored memory {context.memory_id}: {score} - {result.get("reasoning", "")}'
            )
    ...
    async def score_memory(self, context: ScoringContext, method: str = 'hybrid') -> RelevanceScore:
        ...
        if method == 'llm' and self.config.enable_llm_scoring:
            ...
            if scores:
                score_value = float(np.mean(scores))
    ...
    async def apply_reciprocal_rank_fusion(
        self, rankings: dict[str, list[str]], k: Optional[int] = None
    ...
    async def update_memory_feedback(self, memory_id: str, score: RelevanceScore) -> MemoryFeedback:
    ...
    async def _load_feedback(self, memory_id: str) -> Optional[MemoryFeedback]:
        ...
        records, _, _ = await self.driver.execute_query(query, memory_id=memory_id, routing_='r')
