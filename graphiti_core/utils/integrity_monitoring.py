...
class MonitoringThresholds:
    ...
    def from_environment(cls) -> 'MonitoringThresholds':
        ...
        return cls(
            max_entity_count_change_percent=get_float_env(
                'MONITORING_MAX_ENTITY_COUNT_CHANGE_PERCENT', 10.0
            ),
            max_edge_count_change_percent=get_float_env(
                'MONITORING_MAX_EDGE_COUNT_CHANGE_PERCENT', 15.0
            ),
            max_orphaned_edges_percent=get_float_env('MONITORING_MAX_ORPHANED_EDGES_PERCENT', 1.0),
            max_invalid_centrality_percent=get_float_env(
                'MONITORING_MAX_INVALID_CENTRALITY_PERCENT', 2.0
            ),
            max_duplicate_entities_percent=get_float_env(
                'MONITORING_MAX_DUPLICATE_ENTITIES_PERCENT', 0.5
            ),
            max_missing_embeddings_percent=get_float_env(
                'MONITORING_MAX_MISSING_EMBEDDINGS_PERCENT', 5.0
            ),
            max_validation_time_seconds=get_float_env(
                'MONITORING_MAX_VALIDATION_TIME_SECONDS', 30.0
            ),
            min_throughput_entities_per_second=get_float_env(
                'MONITORING_MIN_THROUGHPUT_EPS', 100.0
            ),
            max_temporal_inconsistencies_count=get_int_env(
                'MONITORING_MAX_TEMPORAL_INCONSISTENCIES', 10
            ),
        )
...
class IntegrityMonitoringService:
    ...
    def __init__(
        ...
        self.monitoring_enabled = (
            os.getenv('INTEGRITY_MONITORING_ENABLED', 'true').lower() == 'true'
        )
        self.monitoring_interval = int(
            os.getenv('INTEGRITY_MONITORING_INTERVAL_SECONDS', '300')
        )  # 5 minutes
        self.alert_retention_hours = int(
            os.getenv('MONITORING_ALERT_RETENTION_HOURS', '168')
        )  # 7 days
        ...
        logger.info(
            f'Integrity monitoring service initialized (enabled: {self.monitoring_enabled})'
        )

    def register_custom_check(
        self, name: str, check_func: Callable[[], Awaitable[List[MonitoringAlert]]]
    ):
        ...
        try:
            async with self.driver.session() as session:
                ...
                self.historical_metrics[MonitoringMetric.ENTITY_COUNT].append(
                    {'timestamp': datetime.now(), 'value': current_count}
                )
                ...
                if duplicate_entities > 0 and total_entities > 0:
                    ...
                    if duplicate_percent > self.thresholds.max_duplicate_entities_percent:
                        ...
                        for group in sample_groups[:3]:  # Take first 3 groups
                            sample_entity_ids.extend(
                                group[:3]
                            )  # Take first 3 entities from each group
        ...
        try:
            async with self.driver.session() as session:
                ...
                result = await session.run(
                    "MATCH (n:Entity) WHERE n.name IS NOT NULL AND n.name <> '' RETURN count(n) as total"
                )
                record = await result.single()
                total_entities = record['total'] if record else 0
    ...
    async def run_monitoring_cycle(self) -> HealthReport:
        ...
        critical_count = len(
            [a for a in active_alerts if a.severity == MonitoringSeverity.CRITICAL]
        )
        ...
        logger.info(
            f'Monitoring cycle completed in {cycle_time:.2f}s - Health: {overall_health} '
    ...
    def get_active_alerts(
        self, severity_filter: Optional[MonitoringSeverity] = None
    ...
    def _cleanup_old_alerts(self):
        ...
        if cleaned_count > 0:
            logger.debug(
                f'Cleaned up {cleaned_count} old alerts (retention: {self.alert_retention_hours} hours)'
            )
        ...
        while self.monitoring_enabled:
            try:
                ...
                if health_report.overall_health in ['critical', 'error']:
                    logger.error(
                        f'System health: {health_report.overall_health} - {health_report.total_alerts} active alerts'
                    )
