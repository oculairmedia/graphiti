...
async def atomic_transaction(
    driver: GraphDriver, database: str | None = None
...
async def batch_execute_atomic(
    driver: GraphDriver, queries: list[tuple[str, dict[str, Any]]], database: str | None = None
) -> list[Any]:
    """
...
async def safe_merge_entity(
    driver: GraphDriver, entity_data: dict[str, Any], database: str | None = None
) -> dict[str, Any]:
    """
    ...
    async def merge_operation(tx):
        ...
        existing = await tx.run('MATCH (n:Entity {uuid: $uuid}) RETURN n', uuid=entity_data['uuid'])
...
class TransactionManager:
        ...
        for attempt in range(max_retries + 1):
            try:
                return await execute_atomic_operation(
                    self.driver, operation, *args, database=self.database, **kwargs
                )
    ...
    async def batch_create_entities(self, entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
