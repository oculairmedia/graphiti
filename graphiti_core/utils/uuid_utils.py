...
def normalize_entity_name(name: str, enhanced: bool = True) -> str:
    ...
    return (
        normalized or original_name
    )  # Fallback to original if normalization results in empty string
...
def generate_deterministic_edge_uuid(
    source_uuid: str,
    target_uuid: str,
    name: str,
    group_id: str,
    ...
    """
    Generate a deterministic UUID for an edge based on source, target, name and group_ids.

    This ensures the same edge is not created multiple times between the same nodes,
    while also preventing UUID collisions for cross-group edges (edges connecting
    entities from different group_ids).

    Args:
        source_uuid: Source node UUID
        target_uuid: Target node UUID
        name: Edge name/type
        group_id: Edge group ID (used as namespace for UUID generation)
        source_group_id: Source node's group_id (optional, for cross-group collision prevention)
        target_group_id: Target node's group_id (optional, for cross-group collision prevention)

    Returns:
        Deterministic UUID string

    Note:
        When source_group_id and target_group_id are provided, they are included in the
        edge_key to prevent UUID collisions between edges that connect nodes across
        different groups. This is critical for multi-tenant scenarios where the same
        entity name can exist in multiple groups with different UUIDs.
    """
    # Create a deterministic namespace based on group_id
    ...
    # Build edge_key components
    # Include source/target group_ids when available to prevent cross-group collisions
    key_parts = [source_uuid]
    if source_group_id:
        key_parts.append(source_group_id)
    key_parts.append(target_uuid)
    if target_group_id:
        key_parts.append(target_group_id)
    key_parts.append(normalized_name)

    edge_key = '|'.join(key_parts)
