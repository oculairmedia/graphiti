...
class PostSaveValidator:
            ...
            try:
                ...
                results.append(
                    IntegrityCheckResult.failure(
                        check_name, f'Check failed with exception: {str(e)}', entity_id
                    )
                )
    ...
    async def validate_edge_post_save(
        ...
        for check_name, check_function in self._integrity_checks.items():
            try:
                ...
                results.append(
                    IntegrityCheckResult.failure(
                        check_name, f'Check failed with exception: {str(e)}', edge_id
                    )
                )
    ...
    async def _check_entity_exists(
        ...
        try:
            ...
            return IntegrityCheckResult.success(
                'entity_exists', 'Entity found in database', entity_id
            )

        finally:
            await session.close()

    async def _check_edge_node_references(
        ...
        try:
            ...
            return IntegrityCheckResult.success(
                'edge_node_references', 'Edge references valid nodes', edge_id
            )

        finally:
            await session.close()

    async def _check_uuid_uniqueness(
        self, entity: Any, context: Dict[str, Any]
    ) -> Optional[IntegrityCheckResult]:
    ...
    async def _check_centrality_bounds(
        self, entity: Any, context: Dict[str, Any]
    ) -> Optional[IntegrityCheckResult]:
        ...
        return IntegrityCheckResult.success(
            'centrality_bounds', 'Centrality values are valid', entity_id
        )
    ...
    async def _check_required_fields(
        ...
        return IntegrityCheckResult.success(
            'required_fields', 'All required fields present', entity_id
        )

    async def _check_embedding_consistency(
        self, entity: Any, context: Dict[str, Any]
    ) -> Optional[IntegrityCheckResult]:
        ...
        return IntegrityCheckResult.success(
            'embedding_consistency', 'Embeddings are consistent', entity_id
        )
    ...
    async def _check_temporal_consistency(
        ...
        return IntegrityCheckResult.success(
            'temporal_consistency', 'Timestamps are consistent', entity_id
        )

    async def _check_batch_consistency(
        ...
        return IntegrityCheckResult.success(
            'batch_consistency', f'Batch of {len(entities)} entities is consistent'
        )
...
def get_post_save_validator(driver: GraphDriver) -> PostSaveValidator:
    ...
    if driver_key not in _validator_instances:
        ...
        _validator_instances[driver_key] = PostSaveValidator(
            driver=driver, enable_auto_repair=config['auto_repair']
        )
...
async def run_integrity_repair(
    driver: GraphDriver, results: List[IntegrityCheckResult]
) -> List[str]:
    ...
    for result in results:
        if not result.passed and result.repair_suggestion:
            try:
                ...
                logger.info(
                    f'Attempting repair for {result.check_name}: {result.repair_suggestion}'
                )
