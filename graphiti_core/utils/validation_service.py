...
class CentralizedValidationService:
    ...
    def __init__(
        self, driver: Optional[GraphDriver] = None, config: Optional[ValidationConfig] = None
    ):
        self.driver = driver
    ...
    async def validate_entities_comprehensive(
        self, entities: List[Union[EntityNode, dict]], context: Optional[Dict[str, Any]] = None
    ) -> ValidationReport:
        ...
        try:
            ...
            report.performance_metrics['entities_per_second'] = (
                len(entities) / report.performance_metrics['total_time']
            )
    ...
    async def validate_edges_comprehensive(
        self, edges: List[Union[EntityEdge, dict]], context: Optional[Dict[str, Any]] = None
    ) -> ValidationReport:
        ...
        try:
            ...
            end_time = datetime.now()
            report.performance_metrics['total_time'] = (end_time - start_time).total_seconds()
            report.performance_metrics['edges_per_second'] = (
                len(edges) / report.performance_metrics['total_time']
            )
    ...
    async def validate_post_save(
        ...
        try:
            ...
            integrity_results = await self.post_save_validator.validate_batch_post_save(
                entities,
                context_data,
            )
    ...
    async def _run_pre_save_validation(
        ...
        for i, entity in enumerate(entities):
            ...
            if not result.success:
                severity = (
                    ValidationSeverity.ERROR if result.message else ValidationSeverity.WARNING
                )
        ...
        for i, entity1 in enumerate(entities):
            for j, entity2 in enumerate(entities[i + 1 :], i + 1):
                ...
                if self.fuzzy_matcher.is_entity_match(entity1_dict, entity2_dict):
                    entity1_id = self._get_optional_str(entity1, 'uuid') or f'entity_{i}'
                    entity2_id = self._get_optional_str(entity2, 'uuid') or f'entity_{j}'
    ...
    async def _run_pre_save_edge_validation(
        self, edges: List[Any], report: ValidationReport, context: Dict[str, Any]
    ):
    ...
    async def _run_edge_integrity_validation(
        self, edges: List[Any], report: ValidationReport, context: Dict[str, Any]
    ):
    ...
    def _is_entity(self, obj: Any) -> bool:
        ...
        return isinstance(obj, EntityNode) or (
            ...
            and 'name' in obj
            and 'uuid' in obj
            and 'source_node_uuid' not in obj
        )
    ...
    def _log_validation_report(self, report: ValidationReport):
        ...
        if report.has_errors:
            self.logger.error(
                f'Validation report {report.operation_id}: {report.error_count} errors, {report.warning_count} warnings'
            )
        elif report.warning_count > 0:
            self.logger.warning(
                f'Validation report {report.operation_id}: {report.warning_count} warnings'
            )
        else:
...
def get_validation_service(
    driver: Optional[GraphDriver] = None, config: Optional[ValidationConfig] = None
