...
class MergeStrategy(Enum):
    ...
    PRESERVE_HIGHEST_CENTRALITY = (
        'preserve_highest_centrality'  # Keep entity with highest centrality scores
    )
...
class MergePolicyConfig:
    ...
    field_rules: Optional[Dict[str, FieldMergeRule]] = None
    ...
    def _create_default_field_rules(self) -> Dict[str, FieldMergeRule]:
        ...
        return {
            ...
            'name': FieldMergeRule(
                'name', FieldMergeMode.MERGE, ConflictResolution.LONGEST_WINS, preserve_history=True
            ),
            ...
            'group_id': FieldMergeRule(
                'group_id', FieldMergeMode.PRESERVE, ConflictResolution.FIRST_WINS
            ),
            'created_at': FieldMergeRule(
                'created_at', FieldMergeMode.PRESERVE, ConflictResolution.NUMERIC_MIN
            ),
            'updated_at': FieldMergeRule(
                'updated_at', FieldMergeMode.OVERWRITE, ConflictResolution.NUMERIC_MAX
            ),
            'name_embedding': FieldMergeRule(
                'name_embedding', FieldMergeMode.MERGE, ConflictResolution.LONGEST_WINS
            ),
            # Centrality fields
            'centrality_degree': FieldMergeRule(
                'centrality_degree', FieldMergeMode.MERGE, ConflictResolution.NUMERIC_MAX
            ),
            'centrality_pagerank': FieldMergeRule(
                'centrality_pagerank', FieldMergeMode.MERGE, ConflictResolution.NUMERIC_MAX
            ),
            'centrality_betweenness': FieldMergeRule(
                'centrality_betweenness', FieldMergeMode.MERGE, ConflictResolution.NUMERIC_MAX
            ),
            'centrality_eigenvector': FieldMergeRule(
                'centrality_eigenvector', FieldMergeMode.MERGE, ConflictResolution.NUMERIC_MAX
            ),
            'centrality_importance': FieldMergeRule(
                'centrality_importance', FieldMergeMode.MERGE, ConflictResolution.NUMERIC_MAX
            ),
        }
    ...
    def from_environment(cls) -> 'MergePolicyConfig':
        ...
        def get_enum_env(key: str, enum_class: type, default):
            try:
                ...
                logger.warning(
                    f'Invalid {enum_class.__name__} value for {key}: {value}, using default'
                )
                return default
        ...
        return cls(
            strategy=get_enum_env(
                'MERGE_STRATEGY', MergeStrategy, MergeStrategy.PRESERVE_MOST_COMPLETE
            ),
            default_conflict_resolution=get_enum_env(
            ...
            preserve_entity_with_longest_summary=get_bool_env(
                'MERGE_PRESERVE_LONGEST_SUMMARY', True
            ),
...
class EntityMerger:
    ...
    def _calculate_completeness_score(self, entity: Dict[str, Any]) -> float:
        ...
        if entity.get('summary'):
            score += 2.0 * len(str(entity.get('summary', ''))) / 100
        if entity.get('labels'):
            score += 0.5 * len(entity.get('labels', []))
        if entity.get('name_embedding'):
            score += 1.0
    ...
    def _calculate_centrality_score(self, entity: Dict[str, Any]) -> float:
        ...
        for centrality_field, weight in (self.config.centrality_weights or {}).items():
    ...
    def _merge_entity_data(
        ...
        field_rules = self.config.field_rules or {}
            ...
            if rule.mode == FieldMergeMode.PRESERVE:
                ...
                if values:
                    merged_value = self._resolve_field_conflict(
                        values, rule.conflict_resolution, rule.custom_function
                    )
...
def merge_duplicate_entities(
    ...
    metadata: Optional[Dict[str, Any]] = None,
    config: Optional[MergePolicyConfig] = None,
) -> Dict[str, Any]:
