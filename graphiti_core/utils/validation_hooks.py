...
class ValidationHookRegistry:
    ...
    def register_hook(
        ...
        if existing:
            self.logger.warning(
                f"Hook '{name}' already registered for {hook_type.value}, replacing"
            )
        ...
        self.logger.info(
            f"Registered validation hook '{name}' for {hook_type.value} (priority: {priority})"
        )
    ...
    def execute_hooks(
        ...
        for hook in hooks:
            try:
                ...
                return ValidationResult.failure(f"Hook '{hook.name}' raised exception: {str(e)}")
...
def validate_entity_required_fields(
    entity: Union[EntityNode, dict],
    context: Optional[Dict[str, Any]] = None,
) -> ValidationResult:
    ...
    if isinstance(entity, dict):
        ...
        if missing_fields:
            return ValidationResult.failure(f'Entity missing required fields: {missing_fields}')
    ...
    if isinstance(entity, EntityNode):
        ...
        if normalized_name != entity.name:
            ...
            return ValidationResult.success_with_data(
                entity_dict, f"Normalized name from '{entity.name}' to '{normalized_name}'"
            )
        ...
        if name and isinstance(name, str):
            ...
            if normalized_name != name:
                ...
                return ValidationResult.success_with_data(
                    entity, f"Normalized name from '{name}' to '{normalized_name}'"
                )
...
def detect_entity_duplicates(
    ...
    for i, other_entity in enumerate(batch_entities):
        ...
        if current_uuid and current_uuid == other_uuid:
            return ValidationResult.failure(f'Duplicate UUID detected in batch: {current_uuid}')

        # Check for name+group collision
        if (
            current_name
            and current_group
            and current_name == other_name
            and current_group == other_group
        ):
...
def validate_edge_required_fields(
    ...
    if isinstance(edge, dict):
        ...
        if missing_fields:
            return ValidationResult.failure(f'Edge missing required fields: {missing_fields}')
...
class ValidationService:
    ...
    def __init__(self, registry: Optional[ValidationHookRegistry] = None):
    ...
    def validate_episode(
        self,
        episode: Union[EpisodicNode, dict],
        context: Optional[Dict[str, Any]] = None,
    ) -> ValidationResult:
    ...
    def validate_batch(
        ...
        for i, entity in enumerate(entities):
            ...
            elif isinstance(entity, EpisodicNode) or (
                isinstance(entity, dict) and 'content' in entity
            ):
        ...
        final_result = self.registry.execute_hooks(
            HookType.POST_VALIDATION, validated_entities, context
        )
        return final_result
