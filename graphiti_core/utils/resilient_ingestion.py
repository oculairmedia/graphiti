...
class ResilientIngestionState(BaseModel):
    ...
    def mark_nodes_extracted(self, nodes: list[dict[str, Any]]):
    ...
    def mark_nodes_resolved(self, nodes: list[dict[str, Any]]):
    ...
    def mark_edges_extracted(self, edges: list[dict[str, Any]]):
...
def retry_with_backoff(
    ...
    jitter: bool = True,
    retryable_exceptions: tuple = (RateLimitError, ConnectionError, TimeoutError, asyncio.TimeoutError)
):
    """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        async def wrapper(*args, **kwargs) -> T:
            ...
            effective_max_retries = max_retries if max_retries is not None else int(os.getenv('RESILIENT_RETRY_MAX_ATTEMPTS', '3'))
            effective_base_delay = base_delay if base_delay is not None else float(os.getenv('RESILIENT_RETRY_BASE_DELAY', '2.0'))
            effective_max_delay = max_delay if max_delay is not None else float(os.getenv('RESILIENT_RETRY_MAX_DELAY', '60.0'))
            effective_exponential_base = exponential_base if exponential_base is not None else float(os.getenv('RESILIENT_RETRY_EXPONENTIAL_BASE', '2.0'))
            ...
            for attempt in range(effective_max_retries + 1):
                try:
                    ...
                    if attempt == effective_max_retries:
                        logger.error(f"{func.__name__} failed after {effective_max_retries + 1} attempts: {e}")
                        raise e
                    ...
                    logger.warning(f"{func.__name__} failed on attempt {attempt + 1}/{effective_max_retries + 1}: {e}. "
...
class IngestionProgressCache:
    ...
    def clear_old_states(self, max_age_seconds: float | None = None):
        ...
        effective_max_age = max_age_seconds if max_age_seconds is not None else float(os.getenv('RESILIENT_CACHE_MAX_AGE_SECONDS', '3600'))
        
        current_time = time.time()
        expired_keys = [
            key for key, state in self._cache.items()
