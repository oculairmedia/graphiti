...
class AtomicCentralityStorage:
        ...
        for node_id, node_scores in scores.items():
            ...
            for metric, value in node_scores.items():
                if not isinstance(value, (int, float)):
                    raise ValueError(f'Invalid score type for {node_id}.{metric}: {type(value)}')

                # Centrality scores should be non-negative
                if value < 0:
                    raise ValueError(f'Negative score for {node_id}.{metric}: {value}')

                # PageRank and betweenness should be <= 1
                if metric in ('pagerank', 'betweenness') and value > 1.0:
                    raise ValueError(f'Score out of range for {node_id}.{metric}: {value}')
    ...
    async def _process_batches(
        ...
        for batch_idx in range(0, len(node_ids), self.batch_size):
            ...
            logger.debug(f'Processing batch {batch_num}/{total_batches} ({len(batch_nodes)} nodes)')
    ...
    async def _store_batch_with_retry(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f'Batch storage attempt {attempt + 1} failed: {e}')
...
class BatchUpdateContext:
    ...
    def __init__(self, storage: AtomicCentralityStorage, transaction_id: Optional[str] = None):
        ...
        self._commit_result: Optional[StorageTransaction] = None
    ...
    async def commit(self) -> StorageTransaction:
        ...
        if not self.committed:
            self.committed = True
            self._commit_result = await self.storage.store_centrality_atomic(
                self.scores,
        ...
        raise RuntimeError('Batch commit did not produce a transaction result')
