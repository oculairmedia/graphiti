"""
DEPRECATED: Queue client wrapper for the queued service.
This module is deprecated in favor of Temporal-based ingestion.
Use graphiti_core.utils.temporal_visibility.client.TemporalIngestionClient instead.
...
warnings.warn(
    'graphiti_core.ingestion.queue_client is deprecated. '
    'Use Temporal ingestion via /api/temporal/messages endpoint instead.',
    DeprecationWarning,
    stacklevel=2,
)
...
class QueuedClient:
    ...
    async def push(self, tasks: List[IngestionTask], queue_name: str = 'ingestion') -> List[int]:
        ...
        async with self._get_client() as client:
            ...
            for task in tasks:
                ...
                contents = json.dumps({'priority': task.priority.value, 'task': task.to_json()})
                messages.append(
                    {'contents': contents, 'visibility_timeout_secs': task.visibility_timeout}
                )
    ...
    async def poll(
        self, queue_name: str = 'ingestion', count: int = 10, visibility_timeout: int = 300
    ) -> List[tuple[int, IngestionTask, int]]:
        """
        ...
        async with self._get_client() as client:
            response = await client.post(
                ...
                content=msgpack.packb(
                    {'count': count, 'visibility_timeout_secs': visibility_timeout}
                ),
                headers={'Content-Type': 'application/msgpack'},
            )
            ...
            result_messages = [
                (msg_id, task, poll_tag) for _, msg_id, task, poll_tag in prioritized_messages
            ]
    ...
    async def delete(self, message_id: int, poll_tag: int) -> bool:
        ...
        async with self._get_client() as client:
            response = await client.post(
                ...
                content=msgpack.packb({'messages': [{'id': message_id, 'poll_tag': poll_tag}]}),
                headers={'Content-Type': 'application/msgpack'},
            )
    ...
    async def update(
        self, message_id: int, poll_tag: int, visibility_timeout: int
    ) -> Optional[int]:
        """
        ...
        async with self._get_client() as client:
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f'Updated visibility timeout for message {message_id}, new tag: {new_poll_tag}'
                )
                return new_poll_tag
            else:
