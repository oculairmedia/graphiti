...
def get_vector_cosine_func_query(vec1, vec2, db_type: str = 'neo4j') -> str:
    if db_type == 'falkordb':
        ...
        # UNWIND parameters (edge.*, node.*) are automatically converted by FalkorDB, DON'T wrap them
        def should_wrap_in_vecf32(vec_param: str) -> bool:
            ...
            if vec_param.startswith('$'):
                return True
            # UNWIND parameters (edge.*, node.*, etc.) - FalkorDB handles conversion automatically, DON'T wrap
            if vec_param.startswith(('edge.', 'node.', 'entity.', 'relationship.', 'item.')):
                return False  # Fixed: FalkorDB automatically converts UNWIND list parameters to vectors
