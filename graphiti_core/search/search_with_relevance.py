...
class RelevanceEnhancedSearch:
    ...
    async def search_with_relevance(
        ...
        enhanced_results = await self._enhance_with_relevance(
            results, query, apply_relevance_filter, auto_update_scores
        )

        return enhanced_results

    async def _enhance_with_relevance(
        self, results: SearchResults, query: str, apply_filter: bool, auto_update: bool
    ) -> SearchResults:
        ...
        if results.nodes:
            ...
            enhanced_nodes.sort(
                key=lambda n: n.attributes.get('relevance_score', 0.0), reverse=True
            )
            results.nodes = enhanced_nodes
        ...
        if results.edges:
            ...
            for edge in results.edges:
                ...
                if feedback:
                    ...
                    edge_scores[edge.uuid] = effective_score
            ...
            enhanced_edges.sort(key=lambda e: edge_scores.get(e.uuid, 0.0), reverse=True)
            results.edges = enhanced_edges
    ...
    async def apply_relevance_boost(
        ...
        if results.nodes:
            ...
            results.nodes.sort(key=lambda n: node_scores.get(n.uuid, 0.0), reverse=True)
    ...
    async def apply_rrf_reranking(
        ...
        """
        rankings = {'semantic': semantic_results, 'keyword': keyword_results}
        ...
        return await self.scorer.apply_reciprocal_rank_fusion(rankings, self.config.rrf_k)
