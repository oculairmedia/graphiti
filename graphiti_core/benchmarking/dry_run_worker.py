...
class DryRunWorker:
    ...
    def __init__(self, graphiti: Graphiti, config: DryRunConfig):
        ...
        self.dry_run_driver = create_dry_run_driver(self.original_driver, self.metrics_collector)
    ...
    async def _get_episodes(self) -> List[EpisodicNode]:
        ...
        if self.config.episodes_source == 'database':
            ...
            raise NotImplementedError(
                f"Episodes source '{self.config.episodes_source}' not implemented"
            )

    async def _batch_episodes(
        self, episodes: List[EpisodicNode]
    ) -> AsyncGenerator[List[EpisodicNode], None]:
    ...
    async def _process_episode(self, episode: EpisodicNode) -> EpisodeMetrics:
        ...
        try:
            ...
            try:
                ...
                await self.dry_run_graphiti.add_episode_resilient(
                    name=episode.name,
                ...
                logger.debug(
                    f'Processed episode {episode.uuid} in {episode_metrics.total_duration_ms:.2f}ms'
                )
                ...
                if param_name == 'temperature':
                    if hasattr(self.dry_run_graphiti.llm_client, 'temperature'):
                        original_config['temperature'] = (
                            self.dry_run_graphiti.llm_client.temperature
                        )
    ...
    def _should_terminate(self) -> bool:
        ...
        if self.config.safety.max_runtime_minutes > 0:
            elapsed_minutes = (
                datetime.now() - self.benchmark_metrics.start_time
            ).total_seconds() / 60
        ...
        # Check episode limit
        if (
            self.config.safety.max_episodes_per_run > 0
            and self.benchmark_metrics.total_episodes_processed
            >= self.config.safety.max_episodes_per_run
        ):
            logger.warning(
                f'Episode limit reached: {self.benchmark_metrics.total_episodes_processed}'
            )
            return True
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            writer.writerow(
                [
                    'episode_id',
                    'total_duration_ms',
                    'entity_extraction_time_ms',
                    'edge_extraction_time_ms',
