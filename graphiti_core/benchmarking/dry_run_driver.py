...
class DryRunDriverSession(GraphDriverSession):
    ...
    async def execute_write(self, func, *args, **kwargs):
        ...
        self.metrics_collector.record_write_operation(
            query=f'execute_write: {func.__name__}', params=kwargs
        )
...
class DryRunDriver(GraphDriver):
    ...
    def __init__(
        self, real_driver: 'GraphDriver', metrics_collector: Optional[MetricsCollector] = None
    ):
        super().__init__()
    ...
    async def delete_all_indexes(self, database_: str | None = None) -> None:
        ...
        self.metrics_collector.record_write_operation(
            'CALL db.indexes() YIELD name DROP INDEX name', {'database_': database_}
        )
...
def create_dry_run_driver(
    real_driver: 'GraphDriver', metrics_collector: Optional[MetricsCollector] = None
) -> DryRunDriver:
