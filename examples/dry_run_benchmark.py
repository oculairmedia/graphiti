...
async def run_simple_benchmark():
    ...
    try:
        ...
        logger.info(
            f'Success rate: {results.successful_episodes / results.total_episodes_processed * 100:.1f}%'
        )
        logger.info(
            f'Avg duration: {results.get_summary_stats().get("avg_episode_duration_ms", 0):.1f}ms'
        )
...
async def run_hyperparameter_tuning():
    ...
    for i, hyperparams in enumerate(combinations):
        ...
        try:
            ...
            logger.info(
                f'Run {i + 1} complete - Success rate: {result.successful_episodes / result.total_episodes_processed * 100:.1f}%'
            )

        except Exception as e:
            logger.error(f'Run {i + 1} failed: {e}')
            results.append({'hyperparameters': hyperparams, 'error': str(e)})
        finally:
            await worker.close()
            await graphiti.close()
...
async def run_performance_comparison():
    """Run FalkorDB performance benchmark"""
    logger.info('Running FalkorDB performance benchmark...')
    ...
    config = DryRunConfig(
        run_id=f'backend_comparison_falkordb_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
        description='Performance benchmark using FalkorDB',
        episodes_limit=50,
        batch_size=10,
        max_concurrent_episodes=3,
        output_dir=Path('./benchmark_results/backend_comparison/falkordb'),
        export_formats=['json', 'csv'],
        enable_profiling=True,
        collect_memory_stats=True,
    )
    ...
    try:
        result = await worker.run_benchmark()
        results['FalkorDB'] = result.get_summary_stats()
        logger.info('FalkorDB benchmark complete')
    finally:
        await worker.close()
        await graphiti.close()
...
async def main():
    ...
    parser.add_argument(
        '--hyperparameter-tuning', action='store_true', help='Run hyperparameter tuning'
    )
    parser.add_argument(
        '--backend-comparison', action='store_true', help='Compare database backends'
    )
    parser.add_argument(
        '--episodes-limit', type=int, default=50, help='Number of episodes to process'
    )
