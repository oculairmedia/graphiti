    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": 'Output a JSON object with keys "name" and "type" for the entity "Python programming language"'}
            ],
            max_tokens=100,
...
async def create_graphiti_with_ollama(neo4j_uri: str, neo4j_user: str, neo4j_password: str):
    ...
    ollama_llm_client = OpenAIGenericClient(
        config=llm_config,
        client=llm_client
    )
...
async def main():
    ...
    try:
        # Create Graphiti instance
        graphiti = await create_graphiti_with_ollama(
            neo4j_uri,
            neo4j_user,
            neo4j_password
        )
        ...
        search_results = await graphiti.search_edges(
