...
# Also set level for graphiti_core modules
logging.getLogger('graphiti_core').setLevel(log_level)
logging.getLogger('graphiti_core.ingestion').setLevel(log_level)
...
class WorkerService:
    ...
    async def initialize(self):
        ...
        logger.info(
            f'Connecting to FalkorDB at {falkordb_host}:{falkordb_port}/{falkordb_database}'
        )
        ...
        if os.getenv('USE_CEREBRAS', 'false').lower() == 'true':
            ...
            if not openai_key:
                raise ValueError(
                    'OPENAI_API_KEY environment variable is required when not using Ollama or Cerebras'
                )
