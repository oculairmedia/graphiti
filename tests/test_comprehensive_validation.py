...
class ComprehensiveValidationTestSuite:
    ...
    async def test_database_constraints_integration(self):
        ...
        # Test constraint generation for legacy neo4j backend (for completeness)
        neo4j_constraints = get_unique_constraints('neo4j')
        assert len(neo4j_constraints) > 0, 'Should generate constraints'
        assert any('uuid' in constraint for constraint in neo4j_constraints), (
            'Should include UUID constraints'
        )
        print('✅ Legacy neo4j constraint generation works')
        ...
        assert any('GRAPH.CONSTRAINT CREATE' in constraint for constraint in falkor_constraints), (
            'Should use FalkorDB syntax'
        )
    ...
    async def test_fuzzy_matching_integration(self):
        ...
        edge2 = self.test_data.create_valid_edge(
            source_uuid, target_uuid, fact='John is friends with Mary'
        )
    ...
    async def test_merge_policies_integration(self):
        ...
        entities = [
            self.test_data.create_valid_entity(
                'Alice Smith', created_at=datetime(2023, 1, 1), summary='Data scientist'
            ),
            self.test_data.create_valid_entity(
                'Alice M. Smith',
        ...
        assert len(result['summary']) > len(entities[0]['summary']), (
            'Should preserve more complete summary'
        )
    ...
    async def test_environment_configuration_integration(self):
        ...
        with patch.dict(os.environ, test_env):
            ...
            assert merge_config.strategy == MergeStrategy.PRESERVE_NEWEST, (
                'Should load merge strategy from env'
            )
    ...
    async def test_performance_and_scalability(self):
        ...
        large_entity_batch = [self.test_data.create_valid_entity(f'Entity {i}') for i in range(100)]

        # Add some invalid entities
        large_entity_batch.extend([self.test_data.create_invalid_entity() for _ in range(10)])
        ...
        assert processing_time < 30, (
            f'Should complete within 30 seconds, took {processing_time:.2f}'
        )
        ...
        assert summary['total_entities'] == len(large_entity_batch), (
            'Should count all entities across batches'
        )
