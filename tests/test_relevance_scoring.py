...
class TestQueryLabelRegression:
    """Verify all FalkorDB queries use labeled MATCH for index-backed lookups."""

    @pytest.mark.asyncio
    async def test_load_feedback_uses_labeled_match(self):
        driver = MagicMock()
        driver.execute_query = AsyncMock(return_value=([], None, None))

        scorer = RelevanceScorer(driver=driver, llm_client=MagicMock())
        await scorer._load_feedback('test-uuid')

        query = driver.execute_query.call_args[0][0]
        assert ':Entity' in query, 'MATCH must use :Entity label for index lookup'
        assert ':Episodic' in query, 'MATCH must use :Episodic label for index lookup'
        assert not UNLABELED_MATCH_RE.search(query), f'Unlabeled MATCH found: {query}'

    @pytest.mark.asyncio
    async def test_save_feedback_uses_labeled_match(self):
        driver = MagicMock()
        driver.execute_query = AsyncMock(return_value=([], None, None))
        ...
        assert driver.execute_query.call_count == 2, 'Should call once per label (Entity, Episodic)'
        for call in driver.execute_query.call_args_list:
            query = call[0][0]
            assert ':Entity' in query or ':Episodic' in query, f'Unlabeled MATCH: {query}'
            assert not UNLABELED_MATCH_RE.search(query), f'Unlabeled MATCH found: {query}'
...
class TestRelevanceModels:
    ...
    def test_relevance_score_creation(self):
        ...
        score = RelevanceScore(
            memory_id='test-123', score=0.8, query_id='query-456', scoring_method='llm'
        )
    ...
    def test_memory_feedback_decay(self):
        ...
        feedback = MemoryFeedback(
            memory_id='test-123', avg_relevance=0.8, last_accessed=utc_now() - timedelta(days=30)
        )
...
class TestRelevanceScorer:
    ...
    async def test_score_memory_llm(self, scorer, mock_llm_client):
        ...
        mock_llm_client.generate_response.return_value = json.dumps(
            {'relevance_score': 0.85, 'reasoning': 'Highly relevant to the query'}
        )
    ...
    async def test_score_memory_hybrid(self, scorer, mock_llm_client):
        ...
        # Mock LLM response
        mock_llm_client.generate_response.return_value = json.dumps(
            {'relevance_score': 0.8, 'reasoning': 'Relevant'}
        )
    ...
    async def test_update_memory_feedback(self, scorer, mock_driver):
        ...
        score = RelevanceScore(memory_id='test-123', score=0.75, scoring_method='manual')
...
class TestScoringIntegration:
    ...
    async def test_end_to_end_scoring_workflow(self):
        ...
        mock_llm.generate_response = AsyncMock(
            return_value=json.dumps({'relevance_score': 0.75, 'reasoning': 'Relevant content'})
        )
