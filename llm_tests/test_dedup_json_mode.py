...
TEST_CASES = [
    ...
    },
    {
        ...
        "existing_nodes": [
            {"idx": 0, "name": "Existing Test", "entity_types": ["Test"]}
        ],
        "expected_mappings": {
                ...
                try:
                    ...
                    for res in resolutions:
                        ...
                        if res_id < 0 or res_id > max_extracted_id:
                            bounds_errors.append(f"Invalid id={res_id} (must be 0-{max_extracted_id})")
                        if dup_idx < -1 or dup_idx > max_existing_idx:
                            bounds_errors.append(f"Invalid duplicate_idx={dup_idx} (must be -1 to {max_existing_idx})")
                        ...
                        for res in resolutions:
                            if res.get('id') == expected_id:
                                ...
                                if res.get('duplicate_idx') == expected_dup:
                                    ...
                                    errors.append(f"Entity {expected_id}: expected duplicate_idx={expected_dup}, got {res.get('duplicate_idx')}")
                                break
