...
def print_detailed_results(model: str, results: List[Dict], total_time: float):
    ...
    if successful_tests:
        ...
        avg_precision = sum(r['metrics']['precision'] for r in successful_tests) / len(
            successful_tests
        )
    ...
    for i, result in enumerate(results):
        ...
        if result.get('success'):
            ...
            for cat, scores in metrics['category_scores'].items():
                print(
                    f'     {cat}: {scores["correct"]}/{scores["expected"]} found (extracted {scores["extracted"]})'
                )
...
async def main():
    ...
    slim_results, slim_time = await test_model_extraction(
        base_url, 'slim-ner:q4_k_m', SLIM_NER_PROMPT
    )
    ...
    qwen_results, qwen_time = await test_model_extraction(
        base_url, 'qwen3-30b-a3b:iq4_nl', GENERAL_LLM_PROMPT
    )
