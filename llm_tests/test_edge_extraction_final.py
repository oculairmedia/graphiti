...
TEST_CASES = [
    {
        ...
        'edges': [('John Smith', 'CEO_OF', 'Microsoft'), ('John Smith', 'MARRIED_TO', 'Jane Doe')],
    },
    {
        'text': 'Apple acquired Beats Electronics for $3 billion. Tim Cook announced the deal.',
        'edges': [('Apple', 'ACQUIRED', 'Beats Electronics'), ('Tim Cook', 'ANNOUNCED', 'deal')],
    },
    {
...
[{{"source": "John", "relation": "WORKS_AT", "target": "Google"}}]"""
    ...
    response = await client.post('http://100.81.139.20:11434/api/chat', json=request)
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            if (
                not edges
                and len(data) == 3
                and all(k in data for k in ['source', 'relation', 'target'])
            ):
                ...
                try:
                    ...
                    for edge in edges:
                        if isinstance(edge, dict) and all(
                            k in edge for k in ['source', 'relation', 'target']
                        ):
                            found.add(
                                (
                                    edge['source'].lower(),
                    ...
                    print(
                        f' Done! Time: {elapsed:4.1f}s, Found: {len(edges)}, Correct: {correct}/{len(expected)}'
                    )
