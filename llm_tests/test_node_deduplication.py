            ...
            try:
                ...
                try:
                        ...
                        for res in resolutions:
                            if res.get('id') == expected_id:
                                ...
                                if actual_dup >= len(test_case['existing_nodes']):
                                    errors.append(f"Entity {expected_id}: OUT OF BOUNDS! duplicate_idx={actual_dup} but only {len(test_case['existing_nodes'])} existing nodes")
                                elif actual_dup < -1:
                                    errors.append(f"Entity {expected_id}: INVALID INDEX! duplicate_idx={actual_dup} (should be >= -1)")
                                elif actual_dup == expected_dup:
                                    correct += 1
                                else:
                                    errors.append(f"Entity {expected_id}: expected duplicate_idx={expected_dup}, got {actual_dup}")
                                break
                    ...
                    for node in test_case['existing_nodes']:
                        print(f"  {node['idx']}: {node['name']} ({', '.join(node['entity_types'])})")
                    
                    print(f"\nLLM Response:")
                    for res in resolutions:
                        print(f"  Entity {res.get('id')} -> {res.get('duplicate_idx')} {res.get('name', '')}")
                    ...
                    results.append({
                        ...
                        "has_bounds_errors": any("OUT OF BOUNDS" in e or "INVALID INDEX" in e for e in errors),
                        "errors": errors
                    })
...
async def main():
    ...
    # Save results
    with open(f"dedup_test_results_{model.replace(':', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w') as f:
