...
async def test_model(model: str):
    ...
    regular_result = os.system(
        'cd /opt/stacks/graphiti/llm_tests && python3 test_node_deduplication.py > /dev/null 2>&1'
    )
    ...
    try:
        files = [
            f
            for f in os.listdir('/opt/stacks/graphiti/llm_tests')
            if f.startswith(f'dedup_test_results_{model.replace(":", "_")}')
        ]
        if files:
            ...
            with open(f'/opt/stacks/graphiti/llm_tests/{latest_file}', 'r') as f:
                ...
                results['regular_format'] = {
                    ...
                    'bounds_errors': sum(
                        1 for r in regular_data if r.get('has_bounds_errors', False)
                    ),
                }
    ...
    structured_result = os.system(
        'cd /opt/stacks/graphiti/llm_tests && python3 test_dedup_structured_output.py > /dev/null 2>&1'
    )
    ...
    try:
        files = [
            f
            for f in os.listdir('/opt/stacks/graphiti/llm_tests')
            if f.startswith(f'structured_dedup_results_{model.replace(":", "_")}')
        ]
        if files:
            latest_file = sorted(files)[-1]
            with open(f'/opt/stacks/graphiti/llm_tests/{latest_file}', 'r') as f:
                ...
                results['structured_format'] = {
                    'success': True,
                    'accuracy': sum(r['percentage'] for r in structured_data['results'])
                    / len(structured_data['results']),
                    'bounds_errors': sum(
                        1 for r in structured_data['results'] if r.get('has_bounds_errors', False)
                    ),
                }
    except Exception as e:
...
async def main():
    ...
    with open(comparison_file, 'w') as f:
        json.dump(
            {'timestamp': timestamp, 'models_tested': len(all_results), 'results': all_results},
            f,
            indent=2,
        )
    ...
    for result in all_results:
        ...
        if total_bounds == 0 and any(
            result.get(f, {}).get('success') for f in ['regular_format', 'structured_format']
        ):
