...
TEST_CASES = [
    ...
    },
    {
        ...
        "existing_nodes": [
            {"idx": 0, "name": "Existing Zero", "entity_types": ["Test"]}
        ],
        "expected_mappings": {
                ...
                try:
                    ...
                    for res in resolutions.entity_resolutions:
                        if res.id < 0 or res.id > max_extracted_id:
                            bounds_errors.append(f"Invalid id={res.id} (must be 0-{max_extracted_id})")
                        if res.duplicate_idx < -1 or res.duplicate_idx > max_existing_idx:
                            bounds_errors.append(f"Invalid duplicate_idx={res.duplicate_idx} (must be -1 to {max_existing_idx})")
                        ...
                        for res in resolutions.entity_resolutions:
                            if res.id == expected_id:
                                ...
                                if res.duplicate_idx == expected_dup:
                                    ...
                                    errors.append(f"Entity {expected_id}: expected duplicate_idx={expected_dup}, got {res.duplicate_idx}")
                                break
