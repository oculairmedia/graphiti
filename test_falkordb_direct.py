...
async def test_falkordb():
    ...
    try:
        ...
        relationships = [
            ("MATCH (a:Technology {name: 'FalkorDB'}), (b:Concept {name: 'GraphDatabase'}) CREATE (a)-[:IS_A]->(b)", "FalkorDB IS_A GraphDatabase"),
            ("MATCH (a:Technology {name: 'Neo4j'}), (b:Concept {name: 'GraphDatabase'}) CREATE (a)-[:IS_A]->(b)", "Neo4j IS_A GraphDatabase"),
            ("MATCH (a:Technology {name: 'FalkorDB'}), (b:Language {name: 'Cypher'}) CREATE (a)-[:SUPPORTS]->(b)", "FalkorDB SUPPORTS Cypher"),
            ("MATCH (a:Technology {name: 'Neo4j'}), (b:Language {name: 'Cypher'}) CREATE (a)-[:SUPPORTS]->(b)", "Neo4j SUPPORTS Cypher")
        ]
        ...
        result = graph.query("MATCH (t:Technology)-[:SUPPORTS]->(c:Language {name: 'Cypher'}) RETURN t.name")
        for row in result.result_set:
            print(f"  - {row[0]}")
