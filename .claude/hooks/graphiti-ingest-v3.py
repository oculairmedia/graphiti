...
def read_last_exchange(transcript_path):
    ...
    try:
        ...
        logger.debug(
            f'Found response: {last_assistant_response[:50] if last_assistant_response else "None"}'
        )
...
def main():
    ...
    try:
        ...
        if success:
            logger.info(
                f"Ingested {USER_NAME}'s conversation: {prompt[:50] if prompt else 'No prompt'}..."
            )
        else:
