...
async def create_test_data_directly(neo4j_driver: Neo4jDriver):
    ...
    entities = [
        {"name": "Graph Databases", "type": "Concept", "summary": "Database systems that use graph structures"},
        ...
        {"name": "Knowledge Graphs", "type": "Concept", "summary": "Semantic networks of entities and relationships"},
        {"name": "AI Systems", "type": "Concept", "summary": "Artificial intelligence applications"}
    ]
...
async def verify_migration(neo4j_driver: Neo4jDriver, falkor_driver: FalkorDriver):
    ...
    falkor_node_count = falkor_nodes.records[0]['count'] if falkor_nodes and falkor_nodes.records else 0
...
async def main():
    ...
    try:
        ...
        sample_nodes = await falkor_driver.execute_query("MATCH (n) RETURN n.name, labels(n) LIMIT 5")
